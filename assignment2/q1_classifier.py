...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(dtype=tf.float32, 
                                                shape=[config.batch_size, config.n_features])
        self.labels_placeholder = tf.placeholder(dtype=tf.int32,
                                                shape=[config.batch_size, config.n_classes])
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        m = x.shape[1]
        n = y.shape[1]
        W = tf.Variable(tf.zeros([m, n]), name="weights")
        b = tf.Variable(tf.zeros([N, n]), name="biases")
