...
def make_windowed_data(data, start, end, window_size = 1):
	...
	### YOUR CODE HERE (5-20 lines)
		words = []
		words.append(start * window_size)
		for word in sentence:
			words.append(word)
		words.append(end * window_size)
		# print(words)
		for i in xrange(len(labels)):
			concatWords = []
			for j in xrange(i, i + 2*window_size + 1):
				concatWords.extend(words[j])
			label = labels[i]
			windowed_data.append((concatWords, label))
	# print(windowed_data)
	### END YOUR CODE
...
class WindowModel(NERModel):
	...
	def add_prediction_op(self):
		...
		W = tf.get_variable('W', shape=[self.config.n_window_features * self.config.embed_size, self.config.hidden_size], initializer=initializer(seed=1))
		b1 = tf.get_variable('b1', shape=[self.config.hidden_size,], initializer=initializer(seed=2))
		U = tf.get_variable('U',shape=[self.config.hidden_size, self.config.n_classes], initializer=initializer(seed=3))
		b2 = tf.get_variable('b2', shape=[self.config.n_classes,], initializer=initializer(seed=4))
