...
def pad_sequences(data, max_length):
	...
	for sentence, labels in data:
		...
		if sentence.__len__() <= max_length:
			delta = max_length - sentence.__len__()
			extSentence = sentence + [zero_vector] * delta
			extLabels = labels + [zero_label] * delta
			mask += [False] * delta
		elif sentence.__len__() > max_length:
			extSentence, extLabels, mask = sentence[:max_length], labels[:max_length], mask[:max_length]
		ret.append((extSentence, extLabels, mask))
		### END YOUR CODE ###
	return ret
...
class RNNModel(NERModel):
	...
	def add_prediction_op(self):
		...
		U = tf.get_variable('U', shape=[self.config.hidden_size, self.config.n_classes], initializer=initializer(seed=5))
		...
		with tf.variable_scope("RNN"):
			for time_step in range(self.max_length):
				...
				x_t = x[:, time_step, :]
				o_t, state = cell(x_t, state, scope='RNN')
