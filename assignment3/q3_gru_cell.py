...
class GRUCell(tf.nn.rnn_cell.RNNCell):
	...
	def __call__(self, inputs, state, scope=None):
		...
		with tf.variable_scope(scope):
			...
			W_r = tf.get_variable('W_r', shape=[self._state_size, self._state_size], initializer=initializer, dtype=tf.float32)
			U_r = tf.get_variable('U_r', shape=[self.input_size, self._state_size], initializer=initializer, dtype=tf.float32)
			b_r = tf.get_variable('b_r', shape=[self._state_size,], initializer=tf.constant_initializer(0), dtype=tf.float32)
			W_z = tf.get_variable('W_z', shape=[self._state_size, self._state_size], initializer=initializer, dtype=tf.float32)
			U_z = tf.get_variable('U_z', shape=[self.input_size, self._state_size], initializer=initializer, dtype=tf.float32)
			b_z = tf.get_variable('b_z', shape=[self._state_size,], initializer=tf.constant_initializer(1), dtype=tf.float32)
			W_o = tf.get_variable('W_o', shape=[self._state_size, self._state_size], initializer=initializer, dtype=tf.float32)
			U_o = tf.get_variable('U_o', shape=[self.input_size, self._state_size], initializer=initializer, dtype=tf.float32)
			b_o = tf.get_variable('b_o', shape=[self._state_size,], initializer=tf.constant_initializer(2), dtype=tf.float32)

			x_t = inputs
			h_t_1 = state
			z_t = tf.nn.sigmoid(tf.matmul(x_t, U_z) + tf.matmul(h_t_1, W_z) + b_z)
			r_t = tf.nn.sigmoid(tf.matmul(x_t, U_r) + tf.matmul(h_t_1, W_r) + b_r)
			o_t = tf.nn.tanh(tf.matmul(x_t, U_o) + tf.matmul(r_t*h_t_1, W_o) + b_o)
			h_t = z_t * h_t_1 + (1 - z_t) * o_t
			new_state = h_t
			### END YOUR CODE ###
